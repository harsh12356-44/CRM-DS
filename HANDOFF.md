...
- **Biometric Excel Import & Anup Sen Single-Digit ID Fix**: Resolved single-digit employee ID mapping bug (`normCell.length < 2`) in `src/lib/biometricParser.ts` that skipped Anup Sen (`emp-7` / numeric code `7`). Added `normalizeToISODate()` to format all date inputs into strict `YYYY-MM-DD` ISO dates. Fixed Option 2 (Vertical Flat Date Rows) parser bug where `S.No: 1` caused false 31-day matrix detection. Added `detectMonthYearFromFile()` to scan Excel file contents for month strings (`Sep`, `September`, `-09-`, `/09/`) and automatically target September 2026 (`2026-09`).
...
- **Hostinger Deploy 503 Service Unavailable Resolution**: Updated `.github/workflows/deploy.yml` SSH deployment script to execute `pkill -u $(whoami) -f node || true` and touch `tmp/restart.txt`, cleanly terminating stale process memory and forcing Hostinger to spawn a fresh Node worker process, resolving 503 errors permanently.
